...
**2,029 tests · 94% coverage · 68 architecture decisions · 4 mutation harnesses
proving 18 deliberate breakages are caught**

## How this was built

Most of the code in this repository was generated by an LLM agent working from a
numbered plan I wrote and revised. I directed it, ran the stack, and did the work
the agent could not: the sandbox it authored in could not install the MCP SDK or
run Docker, so every integration — Keycloak, the composed stack, the audit chain
verified from outside the container — was mine, and so was catching what the
agent got wrong when its output met a real machine. The measurements are mine:
the load harness that found `fsync` on the event loop, the ablation runs, the
false-positive rate that demoted two detectors.

I am saying so here because this repository's argument is that its claims are
checkable, and "who wrote this" is a claim. The 2.0.0 release exists because an
external review reproduced two bypasses and a denial-of-service against 1.0.0
that all 1,898 tests had passed — which is the honest state of the art for
agent-generated code, and the reason the six ADRs numbered 0059 and up read the
way they do.
...
Sixty-eight decisions that required thought are recorded in
